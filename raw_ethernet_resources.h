...
#define htobe32_const(x)                                                      \
  (((x) >> 24) | (((x) >> 8) & 0xff00) | ((((x)&0xffffff) << 8) & 0xff0000) | \
 ...
 *		eth_type -
 *		ip_next_protocol -
 *		print_flag - if print_flag == TRUE : print the packet after it's
 *done
 ...
 *				 on bw test, build one packet and duplicate it
 *on
 *the
 *buffer
 *				 on lat test, build only one packet on the
 *buffer
 *(for
 *the ping pong method)
 *
 *	Parameters:
 *				user_param - user_parameters struct for this
 *test
 *				ctx - Test Context.
 *				buf - The QP's packet buffer.
 *				my_dest_info - ethernet information of me
 *				rem_dest_info - ethernet information of the
 *remote
 */
void create_raw_eth_pkt(struct perftest_parameters *user_param,
                        struct pingpong_context *ctx, void *eth_header,
                        struct raw_ethernet_info *my_dest_info,
                        struct raw_ethernet_info *rem_dest_info);

/*calc_flow_rules_size
 * Description: calculate the size of the flow(size of headers - ib, ethernet
 *and ip/udp if available)
 * Parameters:
 *				is_ip_header - if ip header is exist, count the
 *header's size
 *				is_udp_header - if udp header is exist, count
 *the
 *header's size
 *
 */
 ...
 *
 *	Parameters:
 *				flow_rules - Pointer to output, is set to header
 *buffer and specification information
 *				ctx - Test Context.
 *				user_param - user_parameters struct for this
 *test
 *				my_dest_info - ethernet information of me
 ...
 *
 *	Parameters:
 *				flow_rules 	- Pointer to output, is set to
 *header
 *buffer and specification information
 *				ctx 		- Test Context.
 *				user_param 	- user_parameters struct for
 *this
 *test
 *				local_port 	- the local port in the flow
 *rule
 *				remote_port 	- the remote port in the flow
 *rule
 */
 ...
 *this
 *test
 *				allocated_flows	- number of flow ruled that are
 *allocated and ready to be set
 *
 */
