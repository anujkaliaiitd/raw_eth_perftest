...
static enum ibv_wr_opcode opcode_verbs_array[] = {
    IBV_WR_SEND, IBV_WR_RDMA_WRITE, IBV_WR_RDMA_READ};
...
static int pp_init_mmap(struct pingpong_context *ctx, size_t size,
  ...
  ctx->buf[0] =
      mmap(NULL, size, PROT_WRITE | PROT_READ, MAP_SHARED, fd, offset);
  close(fd);
...
static int check_for_contig_pages_support(struct ibv_context *context) {
  ...
  answer = (attr.exp_device_cap_flags &= IBV_EXP_DEVICE_MR_ALLOCATE) ? SUCCESS
                                                                     : FAILURE;
  return answer;
}
/******************************************************************************
...
static int ctx_xrcd_create(struct pingpong_context *ctx,
  ...
  if (ctx->fd < 0) {
    fprintf(stderr, "Error opening file %s errno: %s", tmp_file_name,
            strerror(errno));
    return FAILURE;
  }
...
static int ctx_dc_tgt_create(struct pingpong_context *ctx,
  ...
  if (err) {
    ...
    printf(
        "queried dckry (0x%llx) is different then provided at create "
        "(0x%llx)\n",
...
static struct ibv_qp *ctx_rss_eth_qp_create(
...
int create_rdma_resources(struct pingpong_context *ctx,
  ...
  struct rdma_cm_id **cm_id =
      (user_param->machine == CLIENT) ? &ctx->cm_id : &ctx->cm_id_control;
...
void alloc_ctx(struct pingpong_context *ctx,
  ...
  tarr_size =
      (user_param->noPeak) ? 1 : user_param->iters * user_param->num_of_qps;
  ...
  * with reference to number of flows and number of QPs */
  ...
  if (user_param->connection_type == UD) ctx->buff_size += ctx->cache_line_size;
}
...
int destroy_ctx(struct pingpong_context *ctx,
  ...
  if (user_param->work_rdma_cm == ON) rdma_disconnect(ctx->cm_id);
  ...
  for (i = first; i < user_param->num_of_qps; i++) {
    ...
    if (user_param->connection_type == DC &&
      ...
      if (i == user_param->num_of_qps - 1) return test_result;
...
int create_exp_cqs(struct pingpong_context *ctx,
  ...
  ctx->send_cq =
      ibv_exp_create_cq(ctx->context, tx_buffer_depth * user_param->num_of_qps,
  ...
  if (user_param->verb_type == ACCL_INTF) {
    ...
    ctx->send_cq_family =
        ibv_exp_query_intf(ctx->context, &intf_params, &intf_status);
 ...
 *
 ******************************************************************************/
struct ibv_exp_res_domain *create_res_domain(
    struct pingpong_context *ctx, struct perftest_parameters *user_param) {
...
int create_single_mr(struct pingpong_context *ctx,
  ...
  if (user_param->use_odp) {
    if (!check_odp_support(ctx)) return FAILURE;
...
#if defined(__FreeBSD__)
      posix_memalign(ctx->buf[qp_index], user_param->cycle_buffer,
                     ctx->buff_size);
#else
      if (user_param->use_hugepages) {
...
int alloc_hugepage_region(struct pingpong_context *ctx) {
  int buf_size;
  int alignment = (((ctx->cycle_buffer + HUGEPAGE_ALIGN - 1) / HUGEPAGE_ALIGN) *
                   HUGEPAGE_ALIGN);
...
int ctx_init(struct pingpong_context *ctx,
  ...
  if (user_param->use_hugepages) ctx->is_contig_supported = FAILURE;
  ...
  if (user_param->use_rss) {
    ...
    attr.comp_mask =
        IBV_EXP_DEVICE_ATTR_EXP_CAP_FLAGS | IBV_EXP_DEVICE_ATTR_RSS_TBL_SZ;
    if (ibv_exp_query_device(ctx->context, &attr)) {
...
int create_qp_main(struct pingpong_context *ctx,
  ...
  if (query == 1) user_param->is_exp_qp = 1;
...
struct ibv_qp *ctx_exp_qp_create(struct pingpong_context *ctx,
  ...
  if (user_param->masked_atomics) {
    dev_attr.comp_mask =
        IBV_EXP_DEVICE_ATTR_EXT_ATOMIC_ARGS | IBV_EXP_DEVICE_ATTR_EXP_CAP_FLAGS;
  ...
  if (user_param->inline_recv_size) {
    ...
    attr.sq_sig_all =
        (1 == user_param->cq_mod) ? 1 : 0;  // inline receive on requestor must
                                            // QP's sq_sig_all to be applied
  }
...
static int ctx_modify_qp_to_rtr(struct ibv_qp *qp, struct ibv_qp_attr *attr,
  ...
  if (user_param->connection_type != RawEth) {
    ...
    if (((attr->ah_attr.port_num == user_param->ib_port) &&
        ...
        ((attr->ah_attr.port_num == user_param->ib_port2) &&
         (user_param->gid_index2 == DEF_GID_INDEX) && user_param->dualport)) {
      attr->ah_attr.is_global = 0;
    } else {
      attr->ah_attr.is_global = 1;
      attr->ah_attr.grh.dgid = dest->gid;
  ...
  if (user_param->use_ooo) flags |= ooo_flags;
...
int ctx_connect(struct pingpong_context *ctx, struct pingpong_dest *dest,
  ...
  for (i = 0; i < user_param->num_of_qps; i++) {
    ...
    if (user_param->connection_type == DC) {
      if (ctx_modify_dc_qp_to_rtr(ctx->qp[i], &attr_ex, user_param,
    ...
    if (user_param->tst == LAT || user_param->machine == CLIENT ||
        user_param->duplex) {
      if (user_param->connection_type == DC) {
        if (ctx_modify_dc_qp_to_rts(ctx->qp[i], &attr_ex, user_param,
        ...
        if (user_param->rate_limit_type == PP_RATE_LIMIT) {
          if (ctx_modify_qp_to_rts(ctx->qp[i], &attr_ex, user_param,
                                   &dest[xrc_offset + i], &my_dest[i])) {
    ...
    if (user_param->rate_limit_type == HW_RATE_LIMIT) {
      ...
      if (err || (qp_static_rate != user_param->valid_hw_rate_limit)) {
        if (!user_param->is_rate_limit_type) {
          ...
          fprintf(stderr,
                  "\x1b[31mThe QP failed to accept HW rate limit, "
                  "providing SW rate limit \x1b[0m\n");
        } else {
          fprintf(stderr,
...
void ctx_set_send_exp_wqes(struct pingpong_context *ctx,
  ...
  for (i = 0; i < num_of_qps; i++) {
    ...
    if (user_param->mac_fwd) {
      if (user_param->mr_per_qp) {
        ctx->sge_list[i * user_param->post_list].addr =
            (uintptr_t)ctx->buf[0] +
            (num_of_qps + i) * BUFF_SIZE(ctx->size, ctx->cycle_buffer);
      } else {
        ctx->sge_list[i * user_param->post_list].addr = (uintptr_t)ctx->buf[i];
      }
    }
    ...
    if (user_param->verb == WRITE || user_param->verb == READ)
      ctx->exp_wr[i * user_param->post_list].wr.rdma.remote_addr =
          rem_dest[xrc_offset + i].vaddr;

    else if (user_param->verb == ATOMIC)
      ctx->exp_wr[i * user_param->post_list].wr.atomic.remote_addr =
          rem_dest[xrc_offset + i].vaddr;
    ...
    for (j = 0; j < user_param->post_list; j++) {
      ...
      if (j > 0) {
        ...
        if ((user_param->tst == BW || user_param->tst == LAT_BY_BW) &&
          ...
          increase_loc_addr(&ctx->sge_list[i * user_param->post_list + j],
                            user_param->size, j - 1, ctx->my_addr[i], 0,
      ...
      if (j == (user_param->post_list - 1)) {
        if (user_param->verb_type == ACCL_INTF)
          ctx->exp_wr[i * user_param->post_list + j].exp_send_flags =
        ...
        ctx->exp_wr[i * user_param->post_list + j].next =
            &ctx->exp_wr[i * user_param->post_list + j + 1];
        ctx->exp_wr[i * user_param->post_list + j].exp_send_flags = 0;
      }

      if (user_param->verb == ATOMIC) {
        ctx->exp_wr[i * user_param->post_list + j].exp_opcode =
            exp_opcode_atomic_array[user_param->atomicType];
      } else {
        ctx->exp_wr[i * user_param->post_list + j].exp_opcode =
            exp_opcode_verbs_array[user_param->verb];
      }
      if (user_param->verb == WRITE || user_param->verb == READ) {
        ctx->exp_wr[i * user_param->post_list + j].wr.rdma.rkey =
            rem_dest[xrc_offset + i].rkey;

        if (j > 0) {
          ctx->exp_wr[i * user_param->post_list + j].wr.rdma.remote_addr =
              ctx->exp_wr[i * user_param->post_list + (j - 1)]
                  .wr.rdma.remote_addr;

          if ((user_param->tst == BW || user_param->tst == LAT_BY_BW) &&
              user_param->size <= (ctx->cycle_buffer / 2))
            increase_exp_rem_addr(&ctx->exp_wr[i * user_param->post_list + j],
                                  user_param->size, j - 1, ctx->rem_addr[i],
                                  WRITE, ctx->cache_line_size,
                                  ctx->cycle_buffer);
        }

      } else if (user_param->verb == ATOMIC) {
        ctx->exp_wr[i * user_param->post_list + j].wr.atomic.rkey =
            rem_dest[xrc_offset + i].rkey;

        if (j > 0) {
          ctx->exp_wr[i * user_param->post_list + j].wr.atomic.remote_addr =
              ctx->exp_wr[i * user_param->post_list + j - 1]
                  .wr.atomic.remote_addr;
          if (user_param->tst == BW || user_param->tst == LAT_BY_BW)
            increase_exp_rem_addr(&ctx->exp_wr[i * user_param->post_list + j],
                                  user_param->size, j - 1, ctx->rem_addr[i],
                                  ATOMIC, ctx->cache_line_size,
                                  ctx->cycle_buffer);
        }

        if (user_param->atomicType == FETCH_AND_ADD)
          ctx->exp_wr[i * user_param->post_list + j].wr.atomic.compare_add =
              ATOMIC_ADD_VALUE;

        else
          ctx->exp_wr[i * user_param->post_list + j].wr.atomic.swap =
              ATOMIC_SWAP_VALUE;
        ...
        if (user_param->connection_type == UD) {
          ...
          if (user_param->work_rdma_cm) {
            ctx->exp_wr[i * user_param->post_list + j].wr.ud.remote_qkey =
                user_param->rem_ud_qkey;
            ctx->exp_wr[i * user_param->post_list + j].wr.ud.remote_qpn =
          ...
          ctx->exp_wr[i * user_param->post_list + j].dc.dct_access_key =
              user_param->dct_key;
          ctx->exp_wr[i * user_param->post_list + j].dc.dct_number =
              rem_dest[xrc_offset + i].qpn;
        }
      }
      ...
      if (user_param->use_xrc)
        ctx->exp_wr[i * user_param->post_list + j].qp_type.xrc.remote_srqn =
    ...
    for (j = 0; j < user_param->post_list; j++) {
      ...
      if (user_param->verb == ATOMIC) {
        ctx->wr[i * user_param->post_list + j].opcode =
            opcode_atomic_array[user_param->atomicType];
      } else {
        ctx->wr[i * user_param->post_list + j].opcode =
      ...
      if (user_param->verb == WRITE || user_param->verb == READ) {
        ...
        if (j > 0) {
          ...
          if ((user_param->tst == BW || user_param->tst == LAT_BY_BW) &&
              user_param->size <= (ctx->cycle_buffer / 2))
            increase_rem_addr(&ctx->wr[i * user_param->post_list + j],
                              user_param->size, j - 1, ctx->rem_addr[i], WRITE,
                              ctx->cache_line_size, ctx->cycle_buffer);
        }
        ...
        if (j > 0) {
          ...
          if (user_param->tst == BW || user_param->tst == LAT_BY_BW)
            increase_rem_addr(&ctx->wr[i * user_param->post_list + j],
                              user_param->size, j - 1, ctx->rem_addr[i], ATOMIC,
                              ctx->cache_line_size, ctx->cycle_buffer);
        }

        if (user_param->atomicType == FETCH_AND_ADD)
          ctx->wr[i * user_param->post_list + j].wr.atomic.compare_add =
              ATOMIC_ADD_VALUE;

        else
          ctx->wr[i * user_param->post_list + j].wr.atomic.swap =
              ATOMIC_SWAP_VALUE;
        ...
        if (user_param->connection_type == UD) {
          ...
          if (user_param->work_rdma_cm) {
            ctx->wr[i * user_param->post_list + j].wr.ud.remote_qkey =
                user_param->rem_ud_qkey;
            ctx->wr[i * user_param->post_list + j].wr.ud.remote_qpn =
                user_param->rem_ud_qpn;
      ...
      if (user_param->use_xrc)
        ctx->wr[i * user_param->post_list + j].qp_type.xrc.remote_srqn =
            rem_dest[xrc_offset + i].srqn;
    }
  }
}
...
int ctx_set_recv_wqes(struct pingpong_context *ctx,
  ...
  if (user_param->use_srq) size_per_qp /= user_param->num_of_qps;
  ...
  for (k = 0; i < user_param->num_of_qps; i++, k++) {
    if (!user_param->mr_per_qp) {
      ctx->recv_sge_list[i].addr =
          (uintptr_t)ctx->buf[0] + (num_of_qps + k) * ctx->send_qp_buff_size;
    } else {
    ...
    for (j = 0; j < size_per_qp; ++j) {
      ...
      if ((user_param->tst == BW || user_param->tst == LAT_BY_BW) &&
          user_param->size <= (ctx->cycle_buffer / 2)) {
        increase_loc_addr(&ctx->recv_sge_list[i], user_param->size, j,
                          ctx->rx_buffer_addr[i], user_param->connection_type,
                          ctx->cache_line_size, ctx->cycle_buffer);
      }
    }
    ...
    for (warmindex = 0; warmindex < warmupsession;
      ...
      if (user_param->use_exp == 1)
        err = (ctx->exp_post_send_func_pointer)(
            ctx->qp[index], &ctx->exp_wr[index * user_param->post_list],
            &bad_exp_wr);
      else
...
int run_iter_bw(struct pingpong_context *ctx,
  ...
  /* Will be 0, in case of Duration (look at force_dependencies or in the exp
   * above). */
  ...
  if (user_param->test_type == DURATION && user_param->state != START_STATE &&
      user_param->margin > 0) {
    fprintf(stderr,
            "Failed: margin is not long enough (taking samples before "
            "warmup ends)\n");
  ...
  if (user_param->rate_limit_type == SW_RATE_LIMIT) {
    ...
    switch (user_param->rate_units) {
      case MEGA_BYTE_PS:
        rate_limit_pps =
            ((double)(user_param->rate_limit) / user_param->size) * 1048576;
        break;
      case GIGA_BIT_PS:
        rate_limit_pps =
            ((double)(user_param->rate_limit) / (user_param->size * 8)) *
            1000000000;
        break;
      case PACKET_PS:
  ...
  while (
      totscnt < tot_iters || totccnt < tot_iters ||
    ...
    for (index = 0; index < num_of_qps; index++) {
      ...
      while ((ctx->scnt[index] < user_param->iters ||
        ...
        if (ctx->send_rcredit) {
          uint32_t swindow =
              ctx->scnt[index] + user_param->post_list - ctx->credit_buf[index];
          if (swindow >= user_param->rx_depth) break;
        }
        ...
        if (user_param->verb_type == ACCL_INTF) {
          for (pl_index = 0; pl_index < user_param->post_list; pl_index++) {
            sg_l =
                ctx->exp_wr[index * user_param->post_list + pl_index].sg_list;
            ctx->qp_burst_family[index]->send_pending(
                ctx->qp[index], sg_l->addr, sg_l->length, sg_l->lkey,
          ...
          if (user_param->use_exp == 1) {
            ...
            err = (ctx->post_send_func_pointer)(
                ctx->qp[index], &ctx->wr[index * user_param->post_list],
                &bad_wr);
          }
        }
        if (err) {
        ...
        if (user_param->flows != DEF_FLOWS) {
          if (++flows_burst_iter == user_param->flows_burst) {
            ...
            if (++send_flows_index == user_param->flows) send_flows_index = 0;
            address_offset = send_flows_index * ctx->flow_buff_size;
            ctx->sge_list[0].addr = primary_send_addr + address_offset;
          }
        }

        /* in multiple flow scenarios we will go to next cycle buffer address in
         * the main buffer*/
        if (user_param->post_list == 1 &&
            user_param->size <= (ctx->cycle_buffer / 2)) {
          if (user_param->use_exp == 1)
          ...
          if (user_param->verb != SEND) {
            if (user_param->use_exp == 1)
              increase_exp_rem_addr(&ctx->exp_wr[index], user_param->size,
                                    ...
                                    user_param->verb, ctx->cache_line_size,
                                    ctx->cycle_buffer);
            else
    ...
    if (totccnt < tot_iters ||
      ...
      if (ne > 0) {
        for (i = 0; i < ne; i++) {
          wc_id = (user_param->verb_type == ACCL_INTF) ? 0 : (int)wc[i].wr_id;
          ...
          if (user_param->noPeak == OFF) {
            if (totccnt >= tot_iters - 1)
              user_param->tcompleted[user_param->iters * num_of_qps - 1] =
                  get_cycles();
            else
 ...
 *
 ******************************************************************************/
static inline void set_on_first_rx_packet(
    struct perftest_parameters *user_param) {
  if (user_param->test_type == DURATION) {
    duration_param = user_param;
    user_param->iters = 0;
      ...
      if (ne > 0) {
        ...
        for (i = 0; i < ne; i++) {
          ...
          if (user_param->test_type == DURATION ||
            ...
            if (user_param->verb_type == ACCL_INTF) {
              if (ctx->qp_burst_family[wc_id]->recv_burst(
                      ctx->qp[wc_id], ctx->rwr[wc_id].sg_list, 1)) {
                fprintf(stderr,
              ...
              if (user_param->flows != DEF_FLOWS) {
                if (++recv_flows_burst == user_param->flows_burst) {
                  ...
                  ctx->recv_sge_list[0].addr =
                      primary_recv_addr + address_flows_offset;
                }
              }
            }
            if (SIZE(user_param->connection_type, user_param->size,
                     !(int)user_param->machine) <= (ctx->cycle_buffer / 2)) {
              increase_loc_addr(
                  ctx->rwr[wc_id].sg_list, user_param->size,
          ...
          if (ctx->send_rcredit) {
            ...
            if (credit_cnt % ctx->credit_cnt == 0) {
              ...
              if (ibv_post_send(ctx->qp[wc_id], &ctx->ctrl_wr[wc_id],
                                &bad_wr)) {
...
cleaning:
  if (ctx->send_rcredit) {
    if (clean_scq_credit(tot_scredit, ctx, user_param)) return_value = FAILURE;
  }
  ...
  while (1) {
    ...
    for (index = 0; index < num_of_qps; index++) {
      while ((ctx->scnt[index] - ctx->ccnt[index]) < user_param->tx_depth) {
        if (ctx->send_rcredit) {
          uint32_t swindow = scnt_for_qp[index] + user_param->post_list -
    ...
    if (totccnt < totscnt) {
      ...
      if (ne > 0) {
        for (i = 0; i < ne; i++) {
          if (wc[i].status != IBV_WC_SUCCESS) {
            NOTIFY_COMP_ERROR_SEND(wc[i], ctx->scnt[(int)wc[i].wr_id],
    ...
    if (ne > 0) {
      for (i = 0; i < ne; i++) {
        ...
        if (user_param->use_srq) {
          ...
          if (ibv_post_recv(ctx->qp[wc[i].wr_id], &ctx->rwr[wc[i].wr_id],
                            &bad_wr_recv)) {
...
int run_iter_bi(struct pingpong_context *ctx,
  ...
  struct ibv_send_wr *bad_wr = NULL;
  int num_of_qps = user_param->num_of_qps;
  /* This is to ensure SERVER will not start to send packets before CLIENT start
   * the test. */
  ...
  if (user_param->noPeak == ON) user_param->tposted[0] = get_cycles();
  ...
  while (
    ...
    for (index = 0; index < num_of_qps; index++) {
      while (before_first_rx == OFF &&
             (ctx->scnt[index] < iters || user_param->test_type == DURATION) &&
             ((ctx->scnt[index] + scredit_for_qp[index] - ctx->ccnt[index]) <
              user_param->tx_depth)) {
        if (ctx->send_rcredit) {
          uint32_t swindow =
              ctx->scnt[index] + user_param->post_list - ctx->credit_buf[index];
    ...
    if (ne > 0) {
      ...
      for (i = 0; i < ne; i++) {
        ...
        if (user_param->test_type == DURATION &&
          ...
          if (user_param->report_per_port) {
            user_param
                ->iters_per_port[user_param->port_by_qp[(int)wc[i].wr_id]]++;
          }
          user_param->iters++;
        }

        if (user_param->test_type == DURATION ||
            rcnt_for_qp[wc[i].wr_id] + size_per_qp <= user_param->iters) {
          if (user_param->use_srq) {
            if (ibv_post_srq_recv(ctx->srq, &ctx->rwr[wc[i].wr_id],
                                  &bad_wr_recv)) {
          ...
          if (SIZE(user_param->connection_type, user_param->size,
                   !(int)user_param->machine) <= (ctx->cycle_buffer / 2)) {
            increase_loc_addr(ctx->rwr[wc[i].wr_id].sg_list, user_param->size,
        ...
        if (ctx->send_rcredit) {
          ...
          if (credit_cnt % ctx->credit_cnt == 0) {
            ...
            while ((ctx->scnt[wc[i].wr_id] + scredit_for_qp[wc[i].wr_id] -
                    ctx->ccnt[wc[i].wr_id]) >= user_param->tx_depth) {
            ...
            if (ibv_post_send(ctx->qp[wc[i].wr_id], &ctx->ctrl_wr[wc[i].wr_id],
                              &bad_wr)) {
...
int run_iter_lat_write(struct pingpong_context *ctx,
  ...
  while (scnt < user_param->iters || ccnt < user_param->iters ||
         rcnt < user_param->iters || ((user_param->test_type == DURATION &&
                                       user_param->state != END_STATE))) {
    ...
    if (scnt < user_param->iters || user_param->test_type == DURATION) {
      ...
      *post_buf = (char)++scnt;
      if (user_param->use_exp == 1)
        err = (ctx->exp_post_send_func_pointer)(ctx->qp[0], &ctx->exp_wr[0],
                                                &bad_exp_wr);
      else
...
int run_iter_lat_send(struct pingpong_context *ctx,
  ...
  while (
      scnt < user_param->iters || rcnt < user_param->iters ||
      ((user_param->test_type == DURATION && user_param->state != END_STATE))) {
    /*
     * Get the received packet. make sure that the client won't enter here until
     * he sends
     * his first packet (scnt < 1)
     * server will enter here first and wait for a packet to arrive (from the
     * client)
     */
    if ((rcnt < user_param->iters || user_param->test_type == DURATION) &&
        ...
        if (ne > 0) {
          ...
          if (user_param->test_type == DURATION ||
            ...
            if (user_param->use_srq) {
              ...
              if (ibv_post_recv(ctx->qp[wc.wr_id], &ctx->rwr[wc.wr_id],
                                &bad_wr_recv)) {
            ...
            if (user_param->flows != DEF_FLOWS) {
              if (++recv_flows_index == user_param->flows) {
                ...
                ctx->recv_sge_list[0].addr +=
                    INC(user_param->size, ctx->cache_line_size);
              }
            }
          }
        } else if (ne < 0) {
    ...
    if (scnt < user_param->iters ||
      ...
      if (scnt % user_param->cq_mod == 0 ||
          (user_param->test_type == ITERATIONS && scnt == user_param->iters)) {
        poll = 1;
        if (user_param->use_exp == 1)
          ctx->exp_wr[0].exp_send_flags |= IBV_EXP_SEND_SIGNALED;
        else
      ...
      if (user_param->flows != DEF_FLOWS) {
        if (++send_flows_index == user_param->flows) {
          ...
          ctx->sge_list[0].addr =
              primary_send_addr + (ctx->flow_buff_size * send_flows_index);
        }
      }
      if (poll == 1) {
        struct ibv_wc s_wc;
  ...
  while (totrcnt < (totscnt / user_param->reply_every) || totccnt < tot_iters) {
    ...
    while ((totscnt < user_param->iters) &&
      ...
      if (user_param->post_list == 1 &&
          user_param->size <= (ctx->cycle_buffer / 2)) {
        if (user_param->use_exp == 1)
          increase_loc_addr(ctx->exp_wr[0].sg_list, user_param->size, totscnt,
...
void check_alive(int sig) {
  if (check_alive_data.current_totrcnt > check_alive_data.last_totrcnt) {
    ...
    fprintf(stderr,
            ...
            "Received=%d, Total Iters Required=%d\n",
    ...
    if (check_alive_data.is_events) {
      ...
      fprintf(stderr,
              " Due to this issue, Perftest cannot produce a report "
              "when in event mode.\n");
      exit(FAILURE);
    } else {
...
int run_iter_fs(struct pingpong_context *ctx,
  ...
  if (user_param->test_type == ITERATIONS) {
    ...
    allocated_flows =
        (2 * MAX_FS_PORT) - (user_param->server_port + user_param->client_port);
  }
      ...
      for (flow_index = 0; flow_index < allocated_flows; flow_index++) {
        if (user_param->test_type == ITERATIONS)
          user_param->tposted[tot_fs_cnt] = get_cycles();
                 ...
                 duration_param->state == END_STATE)
          break;
        flow_create_result[flow_index] = ibv_exp_create_flow(
            ctx->qp[qp_index],
