...
static int create_ah_from_wc_recv(struct pingpong_context *ctx,
  ...
  ctx->ah[0] = ibv_create_ah_from_wc(
      ctx->pd, &wc, (struct ibv_grh *)ctx->buf[0], ctx->cm_id->port_num);
...
static int ethernet_read_keys(struct pingpong_dest *rem_dest,
  ...
  if (rem_dest->gid_index == -1) {
    ...
    parsed = sscanf(msg, KEY_PRINT_FMT, (unsigned int *)&rem_dest->lid,
                    &rem_dest->out_reads, &rem_dest->qpn, &rem_dest->psn,
...
static int get_best_gid_index(struct pingpong_context *ctx,
  ...
  for (i = 1; i < attr->gid_tbl_len; i++) {
    ...
    if (is_ipv4_rival && !is_ipv4 && !user_param->ipv6)
      ...
      if (ibv_exp_query_gid_attr(ctx->context, port, i, &gid_attr)) return -1;
      roce_version_rival = gid_attr.type;

      if (check_better_roce_version(roce_version, roce_version_rival) ==
          RIGHT_IS_BETTER)
        gid_index = i;
    }
  }
  ...
  */
  ...
  if (user_param->dualport == ON) {
    if (user_param->gid_index2 != -1) {
      if (ibv_query_port(ctx->context, user_param->ib_port2, &attr)) return 0;
      ...
      if (user_param->use_gid_user) {
        ...
        user_param->gid_index2 =
            get_best_gid_index(ctx, user_param, &attr, user_param->ib_port2);
        if (user_param->gid_index2 < 0) return -1;
        if (ibv_query_gid(ctx->context, user_param->ib_port2,
  ...
  for (i = 0; i < user_param->num_of_qps; i++) {
    ...
    if (user_param->mr_per_qp)
      my_dest[i].vaddr =
          (uintptr_t)ctx->buf[i] + BUFF_SIZE(ctx->size, ctx->cycle_buffer);
    else
    ...
    if ( (user_param->gid_index < 0) ||  ((user_param->gid_index2 < 0) &&
...
int rdma_client_connect(struct pingpong_context *ctx,
  ...
  if (check_add_port(&service, user_param->port, user_param->servername, &hints,
                     &res)) {
    fprintf(stderr, "Problem in resolving basic address and port\n");
    return FAILURE;
  }
...
int retry_rdma_connect(struct pingpong_context *ctx,
  ...
  for (i = 0; i < max_retries; i++) {
    ...
    if (rdma_client_connect(ctx, user_param) == SUCCESS) return SUCCESS;
...
int rdma_server_connect(struct pingpong_context *ctx,
  ...
  if (user_param->work_rdma_cm == ON) alloc_ctx(ctx, user_param);
  ...
  if (user_param->connection_type == UD) conn_param.qp_num = ctx->qp[0]->qp_num;
 ...
 *
 ******************************************************************************/
int ctx_xchg_data_rdma(struct perftest_comm *comm, void *my_data,
                       void *rem_data, int size) {
  if (comm->rdma_params->servername) {
 ...
 *
 ******************************************************************************/
int ctx_xchg_data(struct perftest_comm *comm, void *my_data, void *rem_data,
                  int size) {
  if (comm->rdma_params->use_rdma_cm || comm->rdma_params->work_rdma_cm)
 ...
 *
 ******************************************************************************/
void xchg_bw_reports(struct perftest_comm *comm,
  ...
  if (ctx_xchg_data(comm, (void *)(&temp.bw_avg), (void *)(&rem_bw_rep->bw_avg),
                    sizeof(double))) {
    fprintf(stderr, " Failed to exchange data between server and clients\n");
    exit(1);
  }
...
void ctx_print_pingpong_data(struct pingpong_dest *element,
  ...
  uint16_t dlid = (comm->rdma_params->dlid && comm->rdma_params->side)
                      ? comm->rdma_params->dlid
                      : element->lid;

  if (comm->rdma_params->output != FULL_VERBOSITY) return;
  /*First of all we print the basic format.*/
  printf(BASIC_ADDR_FMT, sideArray[comm->rdma_params->side], dlid, element->qpn,
         element->psn);
  ...
  if ((comm->rdma_params->gid_index > -1 ||
    ...
    printf(PERF_GID_FMT, gidArray[comm->rdma_params->use_mcg && is_there_mgid],
           element->gid.raw[0], element->gid.raw[1], element->gid.raw[2],
           element->gid.raw[3], element->gid.raw[4], element->gid.raw[5],
           element->gid.raw[6], element->gid.raw[7], element->gid.raw[8],
           element->gid.raw[9], element->gid.raw[10], element->gid.raw[11],
           element->gid.raw[12], element->gid.raw[13], element->gid.raw[14],
           element->gid.raw[15]);
  }
}
...
void check_sys_data(struct perftest_comm *user_comm,
  ...
  if (!user_param->dont_xchg_versions) {
    ...
    if (ctx_xchg_data(user_comm, (void *)(&m_cache_line_size),
      ...
      fprintf(stderr,
              " Failed to exchange Cache Line Size data between server "
              "and client\n");
      exit(1);
    }
  }
  ...
  user_param->cache_line_size =
      (rem_cache_line_size > user_param->cache_line_size)
          ? rem_cache_line_size
          : user_param->cache_line_size;
  ...
  if (user_param->connection_type == RawEth) {
    ...
    if (!user_param->dont_xchg_versions) {
      ...
      if (rem_vers >= 5.1) {
        ...
        user_param->curr_mtu = (enum ibv_mtu)(
            (valid_mtu_size(rem_mtu) && (curr_mtu > rem_mtu)) ? rem_mtu
                                                              : curr_mtu);
      } else {
...
int ctx_check_gid_compatibility(struct pingpong_dest *my_dest,
  ...
  if (gid_type1 != gid_type2) return 1;
