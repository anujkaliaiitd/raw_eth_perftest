...
/* This is when we get sig handler from the user before we remove the join
 * request. */
 ...
 * prepare_mcast_mad
 ******************************************************************************/
static void prepare_mcast_mad(uint8_t method, struct mcast_parameters *params,
  ...
  (*(uint16_t *)(ptr + 16)) =
      htons(SUBN_ADM_ATTR_MC_MEMBER_RECORD); /* AttributeID */
...
void set_multicast_gid(struct mcast_parameters *params, uint32_t qp_num,
  ...
  if (is_client && params->user_mgid == NULL) params->mgid.raw[15]++;
}
...
cleanup:
  if (umad_buff) umad_free(umad_buff);
