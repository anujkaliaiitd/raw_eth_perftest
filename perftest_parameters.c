 ...
 * parse_mac_from_str.
 *
 * Description : parse string by format of"XX:XX:XX:XX:XX:XX" to uint8_t array
 *in size 6 for MAC adderes
 *
 *  Parameters :
 *		mac - char*.
...
static int parse_ethertype_from_str(char *ether_str, uint16_t *ethertype_val) {
  ...
  *ethertype_val = strtoul(ether_str, NULL, HEX_BASE);
  if (!*ethertype_val) return FAILURE;
  return SUCCESS;
}
...
/******************************************************************************
  parse_ip_from_str.
 *
 * Description : Convert from presentation format of an Internet number in
 nuffer
 ...
 ******************************************************************************/
int check_if_valid_udp_port(int udp_port) { return ON; }
/******************************************************************************
...
static void usage(const char *argv0, VerbType verb, TestType tst,
  ...
  printf("  -F, --CPU-freq ");
  printf(
      " Do not show a warning even if cpufreq_ondemand module is loaded, "
      "and cpu-freq is not on max.\n");
  ...
  if (tst != FS_RATE) {
    ...
    printf(
        " Set <tos_value> to RDMA-CM QPs. available only with -R flag. "
        "values 0-256 (default off)\n");
  }
  ...
  if (connection_type != RawEth) {
    ...
    printf(
        " Communicate with rdma_cm module to exchange data - use regular "
        "QPs\n");
  }
  ...
  if (tst != FS_RATE) {
    ...
    printf(
        " Perform some iterations before start measuring in order to "
        "warming-up memory cache, valid in Atomic, Read and Write BW "
        "tests\n");
  ...
  if (tst == BW) {
    ...
    if (connection_type != RawEth) {
      ...
      printf(
          " Report BW data on both ports when running Dualport and Duration "
          "mode\n");
    }
  ...
  if (tst == BW || tst == LAT_BY_BW) {
    ...
    printf(
        " Wait <seconds> before destroying allocated resources "
        "(QP/CQ/PD/MR..)\n");

    printf("\n Rate Limiter:\n");
    printf("      --burst_size=<size>");
    printf(
        " Set the amount of messages to send in a burst when using rate "
        "limiter\n");

    printf("      --rate_limit=<rate>");
    printf(
        " Set the maximum rate of sent packages. default unit is [Gbps]. "
        "use --rate_units to change that.\n");

    printf("      --rate_units=<units>");
    printf(
        " [Mgp] Set the units for rate limit to MBps (M), Gbps (g) or pps "
        "(p). default is Gbps (g).\n");
    ...
    printf(
        " [HW/SW/PP] Limit the QP's by HW, PP or by SW. Disabled by "
        "default. When rate_limit Not is specified HW limit is Default.\n");
    printf(
...
void usage_raw_ethernet(TestType tst) {
  ...
  printf("  -E, --dest_mac ");
  printf(
      " destination MAC address by this format XX:XX:XX:XX:XX:XX **MUST** "
      "be entered \n");

  printf("  -G, --use_rss ");
  printf(
      " use RSS on server side. need to open 2^x qps (using -q flag. "
      "default is -q 2). open 2^x clients that transmit to this server\n");

  printf("  -J, --dest_ip ");
  printf(
      " destination ip address by this format X.X.X.X for IPv4 or "
      "X:X:X:X:X:X for IPv6 (using to send packets with IP header)\n");

  printf("  -j, --source_ip ");
  printf(
      " source ip address by this format X.X.X.X for IPv4 or X:X:X:X:X:X "
      "for IPv6 (using to send packets with IP header)\n");

  printf("  -K, --dest_port ");
  printf(
      " destination port number (using to send packets with UDP header as "
      "default, or you can use --tcp flag to send TCP Header)\n");

  printf("  -k, --source_port ");
  printf(
      " source port number (using to send packets with UDP header as "
      "default, or you can use --tcp flag to send TCP Header)\n");
  ...
  printf("  -Z, --server ");
  printf(
      " choose server side for the current machine (--server/--client must "
      "be selected )\n");
  ...
  printf("      --vlan_pcp ");
  printf(
      " specify vlan_pcp value for vlan tag, 0~7. 8 means different "
      "vlan_pcp for each packet\n");

  if (tst != FS_RATE) {
    printf("  -P, --client ");
    printf(
        " choose client side for the current machine (--server/--client "
        "must be selected)\n");
    ...
    printf("      --disable_fcs ");
    printf(
        " Disable Scatter FCS feature. (Scatter FCS is enabled by default "
        "when using --use_exp flag). \n");
...
static void init_perftest_params(struct perftest_parameters *user_param) {
  ...
  user_param->tx_depth = (user_param->tst == BW || user_param->tst == LAT_BY_BW)
                             ? DEF_TX_BW
                             : DEF_TX_LAT;
  ...
  user_param->connection_type =
      (user_param->connection_type == RawEth) ? RawEth : RC;
  ...
  if (user_param->flows != DEF_FLOWS) {
    ...
    if (user_param->test_type == ITERATIONS) {
      ...
      if (user_param->iters / min_iter_req < 1) {
        fprintf(stderr,
                " Current iteration number will not complete full "
                "cycle on all flows, it need to be multiple of the "
                "product between flows and flows_burst\n");
...
static void force_dependecies(struct perftest_parameters *user_param) {
  ...
  if (user_param->test_type == ITERATIONS) {
    ...
    if (user_param->connection_type == UD ||
      ...
      if (user_param->rx_depth == DEF_RX_SEND) {
        user_param->rx_depth =
            (user_param->iters < UC_MAX_RX) ? user_param->iters : UC_MAX_RX;
      }
    }
  }
  ...
  if (user_param->test_method == RUN_ALL) user_param->size = MAX_SIZE;
  ...
  if (user_param->post_list > 1) {
    ...
    printf(RESULT_LINE);
    printf(
        "Post List requested - CQ moderation will be the size of the post "
        "list\n");
  }

  if (user_param->test_type == DURATION) {
    /* When working with Duration, iters=0 helps us to satisfy loop cond. in
    run_iter_bw.
  ...
  if (user_param->connection_type == RawEth) {
    ...
    if (user_param->is_source_mac == OFF) {
      printf(RESULT_LINE);
      fprintf(stderr,
              " Invalid Command line.\n you must enter source mac by "
              "this format -B XX:XX:XX:XX:XX:XX\n");
      exit(1);
    }

    if (user_param->is_dest_mac == OFF &&
        (user_param->tst == LAT ||
         (user_param->machine == CLIENT && !user_param->raw_mcast))) {
      printf(RESULT_LINE);
      fprintf(stderr,
              " Invalid Command line.\n you must enter dest mac by "
              "this format -E XX:XX:XX:XX:XX:XX\n");
      exit(1);
    }
    ...
    if ((user_param->is_server_port == ON &&
      ...
      printf(RESULT_LINE);
      fprintf(stderr,
              " Invalid Command line.\n if you would like to send UDP "
              "header,\n you must enter server&client port "
              "--server_port X --client_port X\n");
      exit(1);
    }

    if ((user_param->is_server_port == ON) &&
        (user_param->is_server_ip == OFF || user_param->is_client_ip == OFF)) {
      printf(RESULT_LINE);
      fprintf(stderr,
              " Invalid Command line.\nPlease provide source_ip and/or "
              "dest_ip when using UDP\n");
      exit(1);
    }
    ...
    if (user_param->tcp == ON && user_param->is_server_port == OFF) {
      printf(RESULT_LINE);
      fprintf(stderr,
              "Invalid Command line.\nPlease provide UDP information "
              "(IP & UDP Port src/dest) in order to use TCP\n");
      exit(1);
    }
    ...
    if (user_param->duplex == OFF && user_param->mac_fwd == ON) {
      printf(
          "mac_fwd should run in duplex mode only. changing to duplex mode.\n");
  ...
  if (user_param->use_mcg) {
    if (user_param->connection_type != UD) user_param->connection_type = UD;
  ...
  if (user_param->test_method == RUN_INFINITELY) {
    ...
    if (user_param->duplex && user_param->verb == SEND) {
      printf(RESULT_LINE);
      fprintf(stderr,
              " run_infinitely mode is not supported in SEND "
  ...
  if (user_param->rate_limit_type == SW_RATE_LIMIT) {
    if (user_param->tst != BW || user_param->verb == ATOMIC ||
      ...
      printf(RESULT_LINE);
      fprintf(stderr,
              "SW Rate limiter cann't be executed on non-BW, ATOMIC or "
              "bidirectional SEND tests\n");
      exit(1);
    }
    ...
    switch (user_param->rate_units) {
      case MEGA_BYTE_PS:
        rate_limit_gbps = ((
            double)(((user_param->rate_limit) * 8 * 1024 * 1024) / 1000000000));
        break;
      case GIGA_BIT_PS:
      ...
      case PACKET_PS:
        printf(RESULT_LINE);
        fprintf(stderr,
                " Failed: pps  rate limit units is not supported when "
                "setting HW rate limit\n");
        exit(1);
      default:
        printf(RESULT_LINE);
  ...
  if (user_param->tst == LAT_BY_BW) {
    if (user_param->test_type == DURATION) {
      fprintf(stderr,
              "Latency under load test is currently support iteration "
              "mode only.\n");
      exit(1);
    }
    if (user_param->num_of_qps > 1) {
  ...
  if (user_param->output != FULL_VERBOSITY) {
    if (user_param->tst == BW &&
        !(user_param->output == OUTPUT_BW || user_param->output == OUTPUT_MR)) {
      printf(RESULT_LINE);
      fprintf(
          stderr,
  ...
  if (user_param->use_cuda && user_param->mmap_file != NULL) {
    printf(RESULT_LINE);
    fprintf(stderr,
            "You cannot use CUDA and an mmap'd file at the same time\n");
    exit(1);
  }
#endif
  ...
  if (user_param->verb_type == ACCL_INTF) {
    if (user_param->connection_type != RC &&
        ...
        user_param->connection_type != RawEth) {
      fprintf(stderr,
              "Accelerated verbs support RC/UC/RAW_ETH connections only.\n");
      exit(1);
    }
    ...
    if (user_param->num_of_qps > 1) {
      fprintf(stderr,
              "Accelerated verbs in perftest support only 1 qp for now.\n");
      exit(1);
    }
    ...
    if (user_param->tst != BW) {
      fprintf(
          stderr,
          "Accelerated verbs in perftest supports only BW tests for now.\n");
      exit(1);
    }
    ...
    if (user_param->duplex) {
      fprintf(
          stderr,
          "Accelerated verbs in perftest supports only unidir tests for now\n");
      exit(1);
    }

    if (user_param->test_method == RUN_INFINITELY) {
      fprintf(stderr,
              "Accelerated verbs in perftest does not support Run "
              "Infinitely mode for now\n");
      exit(1);
    }
  }
  if (user_param->perform_warm_up &&
      !(user_param->tst == BW &&
        (user_param->verb == ATOMIC || user_param->verb == WRITE ||
         user_param->verb == READ))) {
    fprintf(
        stderr,
        "Perform warm up is available in ATOMIC, READ and WRITE BW tests only");
    exit(1);
  }
...
static int set_link_layer(struct ibv_context *context,
  ...
  if (curr_link == LINK_UNSPEC) params->link_type = port_attr.link_layer;
  ...
  if (params->dualport == ON) {
    ...
    if (curr_link == LINK_UNSPEC) params->link_type2 = port_attr.link_layer;
  ...
  if (user_param->inline_size == DEF_INLINE) {
    if (user_param->tst == LAT) {
      switch (user_param->verb) {
        case WRITE:
          user_param->inline_size = (user_param->connection_type == DC)
                                        ? DEF_INLINE_DC
                                        : DEF_INLINE_WRITE;
          break;
        case SEND:
...
void set_raw_eth_parameters(struct perftest_parameters *user_param) {
  ...
  if (user_param->is_new_raw_eth_param == 1 &&
    ...
    printf(RESULT_LINE);
    fprintf(stderr,
            " Invalid Command line.\nMix of source with local|remote "
            "and dest with local|remote is not supported.\n");
    fprintf(stderr,
            "For L2 tests you must enter local and remote mac  by this "
            "format --local_mac XX:XX:XX:XX:XX:XX --remote_mac "
            "XX:XX:XX:XX:XX:XX\n");
    fprintf(stderr,
            "For L3 tests You must add also local and remote ip  by "
            "this format --local_ip X.X.X.X --remote_ip X.X.X.X\n");
    fprintf(stderr,
            "For L4 you need to add also local and remote port  by "
            "this format --local_port XXXX  --remote_port XXXX\n");
    exit(1);
  }
...
int parser(struct perftest_parameters *user_param, char *argv[], int argc) {
  ...
  if (user_param->connection_type == RawEth) user_param->machine = UNCHOSEN;
  ...
  while (1) {
    static const struct option long_options[] = {
      ...
      {.name = "wait_destroy",
       .has_arg = 1,
       .flag = &wait_destroy_flag,
       .val = 1},
      {.name = "disable_fcs",
       .has_arg = 0,
       .flag = &disable_fcs_flag,
       .val = 1},
      {.name = "flows", .has_arg = 1, .flag = &flows_flag, .val = 1},
      {.name = "flows_burst",
       .has_arg = 1,
       .flag = &flows_burst_flag,
       .val = 1},
      {.name = "reply_every",
       .has_arg = 1,
       .flag = &reply_every_flag,
       .val = 1},
      {.name = "perform_warm_up",
       .has_arg = 0,
       .flag = &perform_warm_up_flag,
       .val = 1},
    ...
    c = getopt_long(argc, argv,
                    "w:y:p:d:i:m:s:n:t:u:S:x:c:q:I:o:M:r:Q:A:l:D:f:"
                    "B:T:E:J:j:K:k:X:aFegzRvhbNVCHUOZP",
                    long_options, NULL);
    ...
    switch (c) {
      ...
      case 'd':
        ...
        break;
      case 'i':
        CHECK_VALUE(user_param->ib_port, uint8_t, MIN_IB_PORT, MAX_IB_PORT,
                    "IB Port");
        break;
      case 'm':
        ...
        break;
      case 'n':
        CHECK_VALUE(user_param->iters, int, MIN_ITER, MAX_ITER,
                    "Iteration num");
        break;
      case 't':
        CHECK_VALUE(user_param->tx_depth, int, MIN_TX, MAX_TX, "Tx depth");
        break;
      case 'T':
        CHECK_VALUE(user_param->tos, int, MIN_TOS, MAX_TOS, "TOS");
        break;
      case 'u':
      ...
      case 'S':
        ...
        if (user_param->connection_type == RawEth) user_param->raw_qos = 1;
        break;
      case 'x':
        CHECK_VALUE(user_param->gid_index, uint8_t, MIN_GID_IX, MAX_GID_IX,
                    "Gid index");
      ...
      case 'q':
        ...
        break;
      case 'I':
        CHECK_VALUE(user_param->inline_size, int, 0, MAX_INLINE, "Max inline");
        if (user_param->verb == READ || user_param->verb == ATOMIC) {
          fprintf(stderr,
      ...
      case 'o':
        ...
        break;
      case 'M':
        GET_STRING(user_param->user_mgid, strdupa(optarg));
        break;
      case 'r':
        CHECK_VALUE(user_param->rx_depth, int, MIN_RX, MAX_RX, " Rx depth");
        ...
        break;
      case 'Q':
        CHECK_VALUE(user_param->cq_mod, int, MIN_CQ_MOD, MAX_CQ_MOD,
                    "CQ moderation");
        break;
      case 'A':
        if (user_param->verb != ATOMIC) {
          ...
          fprintf(stderr,
                  " To change the atomic action type, you must run one "
                  "of the atomic tests\n");
          return 1;
        }
        ...
        if (strcmp(atomicTypesStr[0], optarg) == 0)
          ...
          fprintf(stderr,
                  " Invalid Atomic type! please choose from "
      ...
      case 'B':
        ...
        if (parse_mac_from_str(optarg, user_param->source_mac)) return FAILURE;
        break;
      case 'E':
        user_param->is_old_raw_eth_param = 1;
        user_param->is_dest_mac = ON;
        if (parse_mac_from_str(optarg, user_param->dest_mac)) return FAILURE;
        break;
      case 'J':
        user_param->is_old_raw_eth_param = 1;
      ...
      case 0: /* required for long options to work. */
        ...
        if (verbosity_output_flag) {
          if (strcmp("bandwidth", optarg) == 0) {
            ...
            fprintf(stderr,
                    " Invalid verbosity level output flag. Please use "
                    "bandwidth, latency, message_rate\n");
            return FAILURE;
          }
        ...
        if (local_mac_flag) {
          ...
          if (parse_mac_from_str(optarg, user_param->local_mac)) return FAILURE;
          local_mac_flag = 0;
        }
        if (remote_ip_flag) {
          user_param->is_new_raw_eth_param = 1;
  ...
  if (raw_ipv6_flag) {
    if (user_param->is_new_raw_eth_param) {
      if (user_param->is_server_ip) {
        if (1 != parse_ip6_from_str(
                     local_ip, (struct in6_addr *)&(user_param->local_ip6))) {
      ...
      if (user_param->is_client_ip) {
        if (1 != parse_ip6_from_str(
                     remote_ip, (struct in6_addr *)&(user_param->remote_ip6))) {
      ...
      if (user_param->is_server_ip) {
        if (1 != parse_ip6_from_str(
                     server_ip, (struct in6_addr *)&(user_param->server_ip6))) {
      ...
      if (user_param->is_client_ip) {
        if (1 != parse_ip6_from_str(
                     client_ip, (struct in6_addr *)&(user_param->client_ip6))) {
    ...
    if (user_param->is_new_raw_eth_param) {
      if (user_param->is_server_ip) {
        if (1 != parse_ip_from_str(local_ip, &(user_param->local_ip))) {
          fprintf(stderr, " Invalid local IP address\n");
          return FAILURE;
        }
      }
      if (user_param->is_client_ip) {
        if (1 != parse_ip_from_str(remote_ip, &(user_param->remote_ip))) {
          fprintf(stderr, " Invalid remote IP address\n");
          return FAILURE;
        }
      }
      ...
      if (user_param->is_server_ip) {
        if (1 != parse_ip_from_str(server_ip, &(user_param->server_ip))) {
          fprintf(stderr, " Invalid server IP address\n");
          return FAILURE;
        }
      }
      if (user_param->is_client_ip) {
        if (1 != parse_ip_from_str(client_ip, &(user_param->client_ip))) {
          fprintf(stderr, " Invalid client IP address\n");
          return FAILURE;
        }
      }
  ...
  if (use_ooo_flag) user_param->use_ooo = 1;
  if (vlan_en) {
    user_param->vlan_en = ON;
...
int check_link_and_mtu(struct ibv_context *context,
  ...
  if (user_param->connection_type == RawEth) {
    ...
    user_param->curr_mtu =
        set_mtu(context, user_param->ib_port, user_param->mtu);
  }
...
void ctx_print_test_info(struct perftest_parameters *user_param) {
  ...
  if (user_param->output != FULL_VERBOSITY) return;
  ...
  if (user_param->use_mcg) printf("Multicast ");
  ...
  if (user_param->use_mcg) printf(" MultiCast runs on UD!\n");
  ...
  if (user_param->use_rdma_cm) temp = 1;
...
static float calc_cpu_util(struct perftest_parameters *user_param) {
  ...
  ustat_diff =
      user_param->cpu_util_data.ustat[1] - user_param->cpu_util_data.ustat[0];
  idle_diff =
      user_param->cpu_util_data.idle[1] - user_param->cpu_util_data.idle[0];
...
void print_report_bw(struct perftest_parameters *user_param,
  ...
  opt_delta =
      user_param->tcompleted[opt_posted] - user_param->tposted[opt_completed];
  ...
  run_inf_bi_factor =
      (user_param->duplex && user_param->test_method == RUN_INFINITELY)
          ? (user_param->verb == SEND ? 1 : 2)
          : 1;
  tsize = run_inf_bi_factor * user_param->size;
  num_of_calculated_iters *=
      (user_param->test_type == DURATION) ? 1 : num_of_qps;
  ...
  sum_of_test_cycles =
      ((double)(user_param->tcompleted[location_arr] - user_param->tposted[0]));

  double bw_avg = ((double)tsize * num_of_calculated_iters * cycles_to_units) /
                  (sum_of_test_cycles * format_factor);
  double msgRate_avg =
      ((double)num_of_calculated_iters * cycles_to_units * run_inf_bi_factor) /
      (sum_of_test_cycles * 1000000);

  double bw_avg_p1 =
      ((double)tsize * user_param->iters_per_port[0] * cycles_to_units) /
      (sum_of_test_cycles * format_factor);
  double msgRate_avg_p1 = ((double)user_param->iters_per_port[0] *
                           cycles_to_units * run_inf_bi_factor) /
                          (sum_of_test_cycles * 1000000);

  double bw_avg_p2 =
      ((double)tsize * user_param->iters_per_port[1] * cycles_to_units) /
      (sum_of_test_cycles * format_factor);
  double msgRate_avg_p2 = ((double)user_param->iters_per_port[1] *
                           cycles_to_units * run_inf_bi_factor) /
                          (sum_of_test_cycles * 1000000);
...
void print_full_bw_report(struct perftest_parameters *user_param,
  ...
  if (user_param->output == FULL_VERBOSITY) {
    fflush(stdout);
    fprintf(stdout,
            user_param->cpu_util_data.enable ? REPORT_EXT_CPU_UTIL : REPORT_EXT,
...
void print_report_lat(struct perftest_parameters *user_param) {
  ...
  measure_cnt = (user_param->tst == LAT)
                    ? user_param->iters - 1
  ...
  if (user_param->r_flag->unsorted || user_param->r_flag->histogram) {
    if (user_param->output == FULL_VERBOSITY) {
      printf(RESULT_LINE);
      printf("%s",
             (user_param->test_type == ITERATIONS) ? RESULT_FMT_LAT
                                                   : RESULT_FMT_LAT_DUR);
  ...
  if (user_param->output == OUTPUT_LAT)
    ...
    printf(REPORT_FMT_LAT, (unsigned long)user_param->size, user_param->iters,
    ...
    printf(user_param->cpu_util_data.enable ? REPORT_EXT_CPU_UTIL : REPORT_EXT,
           calc_cpu_util(user_param));
  }
...
void print_report_lat_duration(struct perftest_parameters *user_param) {
  ...
  if (user_param->output == OUTPUT_LAT) {
    ...
    printf(REPORT_FMT_LAT_DUR, user_param->size, user_param->iters, latency,
           tps);
    printf(user_param->cpu_util_data.enable ? REPORT_EXT_CPU_UTIL : REPORT_EXT,
           calc_cpu_util(user_param));
  }
...
void print_report_fs_rate(struct perftest_parameters *user_param) {
  ...
  if (user_param->test_type == ITERATIONS) {
    ...
    /* calcualte average sum on sorted array*/
    for (i = 0; i < measure_cnt; ++i) average_sum += delta[i];
  ...
  if (user_param->output == FULL_VERBOSITY) {
    printf(RESULT_LINE);
    printf("%s",
           (user_param->test_type == ITERATIONS) ? RESULT_FMT_FS_RATE
  ...
  if (user_param->output == OUTPUT_LAT)
    ...
    if (user_param->test_type == ITERATIONS) {
      ...
      printf(REPORT_FMT_FS_RATE, user_param->iters, delta[0] / cycles_to_units,
             delta[measure_cnt - 1] / cycles_to_units, latency, average, fps);
    } else {
      printf(REPORT_FMT_FS_RATE_DUR, user_param->iters, latency, fps);
    }
    printf(user_param->cpu_util_data.enable ? REPORT_EXT_CPU_UTIL : REPORT_EXT,
           calc_cpu_util(user_param));
  }
